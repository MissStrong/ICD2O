...
function setup() {
  ...
  background(255);
  fill(0);
  while (x < 600) { // the next two lines will keep repeating as long as coordinate < 600
    rect(x, x, 50); 
    x += 50;
  }
  // right at this point, everything shows up on the canvas
...
function setup() {
  ...
  background(255);
  fill(0);
  while (0 <= x) { // the next two lines will keep repeating as long as 0 <= coordinate (which is always!)
    rect(x, x, 50); 
    x += 50;
  }
