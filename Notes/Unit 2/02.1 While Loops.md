...
In the previous example, each line is almost identical with the exception of the coordinates. What we can do is store the coordinate as a variable and repeatedly call `rect(x, x, 50)` and `x += 50`.
...
function setup() {
  ...
  background(255);
  fill(0);
  while (x < 600) { // the next two lines will keep repeating as long as x < 600
    rect(x, x, 50); 
    x += 50;
  }
  // right at this point, everything shows up on the canvas
